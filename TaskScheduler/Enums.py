...
class TaskStage(models.TextChoices):
	# Uploading       = ("1-UPL", "Uploading")  # can't recover in this stage
	Pending         = ("1-PEN", "Pending")  # waiting for Worker to be assigned to
	# Distributing    = ("3-DIS", "Distributing")  # can't differentiate as task can be distributed and rendered at the same time
	Rendering       = ("2-REN", "Rendering")
	Concatenating   = ("3-CON", "Concatenating")
	Finished        = ("4-FIN", "Finished")
	Expired         = ("5-EXP", "Expired")  # task result deleted from Server

	def base_progress(self):
		return max(int(self.as_number()) / 4, 1.0)
...
class SubtaskStage(models.TextChoices):
	...
	Transferring 	= ("2-TRA", "Transferring")
	Running     	= ("3-RUN", "Running")
	Finished    	= ("4-FIN", "Finished")
	Aborted     	= ("5-ABO", "Aborted")
